...
class ResNetBackbone(nn.Module):
    def __init__(self,
                 ...
                 pretrained=True):
        """
        This is the backbone of resnet
        :param in_channels:
        ...
        assert self.arch in Arches.keys(), 'Invalid value of arch {}'.format(self.arch)
        model = Arches[self.arch](pretrained)
        del model.avgpool
        del model.fc
        if in_channels > 3:
            ...
            model.conv1.weight = torch.nn.Parameter(weight_expansion)
        if arch == 'resnet50':
