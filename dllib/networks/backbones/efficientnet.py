...
class EfficientNet(nn.Module):
    ...
    def extract_endpoints(self, inputs):
        ...
        for idx, block in enumerate(self._blocks):
            ...
            if block._depthwise_conv.prev_return:
    ...
    def _change_block_stride_dilation(self, idx, stride=1, dilation=1):
        """Adjust block's first convolution layer to stride and dilation.

        Args:
            idx (int): Block index.
            stride (int, list): stride.
            dilation (int, list): dilation.
        """
        ...
        # kernel = self._blocks[idx]._depthwise_conv
        module = self._blocks[idx]._depthwise_conv
        ...
        if pad_h > 0 or pad_w > 0:
            module.static_padding = nn.ZeroPad2d((pad_w // 2, pad_w - pad_w // 2,
                                                  pad_h // 2, pad_h - pad_h // 2))
        else:
            module.static_padding = nn.Identity()
        self._blocks[idx]._depthwise_conv.stride = stride
        self._blocks[idx]._depthwise_conv.dilation = dilation
...
class EfficientNetBackbone(nn.Module):
    def __init__(self,
        ...
        self.layer_index_list = LAYER_INDEX_LISTS[arch]
        ...
        assert len(self.layer_index_list) == len(self.layer_return_list), \
            'layer_index_list and layer_return_list should have same length'
        if pretrained:
            model = EfficientNet.from_pretrained(arch)
        else:
            model = EfficientNet.from_name(arch)
        for index, stride in zip(self.layer_index_list, self.layer_stride_list):
            if stride == 1:
                model._change_block_stride_dilation(index, 1, 2)
        self.model = model
        self.arch = arch

    def forward(self, x):
        feature_list = self.model.extract_endpoints(x)
        feature_maps = []
        if self.return_layer0:
            feature_maps.append(feature_list['reduction_1'])
        for i in range(4):
            if self.layer_return_list[i]:
                feature_maps.append(feature_list['reduction_%d' % (i + 2)])
        return feature_maps
